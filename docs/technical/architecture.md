...
the heads of the operation log are stored by the "op heads" backend, and the
working copy is stored by the working copy backend. The last two don't have
their own traits defined yet, but their interfaces are small and easy to create
traits for when needed. All the interfaces are defined in terms of plain Rust
data types, not tied to a specific format. The commit index, however, has an
interface that is closely tied to the on-disk format. We will need to clean that
up soon.
...
size but the values can have different sizes. We use a custom format because we
want [lock-free concurrency](concurrency.md).
...
Copied from Mercurial, but I experimented with the syntax. Don't really like it.

### Diff-editing

Very sparse checkout
